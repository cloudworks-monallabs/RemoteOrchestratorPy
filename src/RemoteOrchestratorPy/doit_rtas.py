...
# Create a defaultdict where each value is a dict
...
dangling_tasks = defaultdict(dict)

def get_dangling_task( TASKGENFUNC, ipv6):
    assert TASKGENFUNC.__name__ in dangling_tasks
    return dangling_tasks[TASKGENFUNC.__name__][ipv6]

def doit_taskify(all_rtas,
                 **kwargs
                 ):
    """
    ...
    def wrapper(func):
        ...
        set_dangling_task_dep = kwargs.pop("set_dangling_task_dep", None)
        ...
        def doit_task():
            ...
            for rtas in all_rtas:
                # generate tasks only if previous rtas has been successfull 
                if rtas.task_failed_abort_execution == False:
                    # specify rtaswise task dependecies
                    task_dep = []
                    if set_dangling_task_dep:
                        task_dep = [task for _tg in set_dangling_task_dep for task in get_dangling_task( _tg, rtas.ipv6)]
                        print("set dangling_tasks = ", task_dep)

                    rtas.set_super_task_seq(rtas_basename, task_dep=task_dep)
            ...
            # by default an rtas sequence 
            if kwargs.get("leaf_barrier_task", True):
                print(f"adding leaf barrier {rtas_basename}: ", all_rtas_tasks)
                trec = {'basename': rtas_basename,
                        'name': "_leaf_final_",
                        'actions': None,
                ...
                dangling_tasks[rtas_basename][rtas.ipv6] = all_rtas_tasks

        doit_task.create_doit_tasks = doit_task
        return doit_task
    #wrapper.create_doit_tasks = wrapper
...
class RemoteTaskActionSequence:
    ...
    def set_task_local_step_pre(self,
        ...
        **kwargs has task like targets, file_dep, uptodate e
        ...
        print("task_dep for local_step_pre = ", trec['task_dep'])
        self.task_dep = []

        self.task_local_step_pre = trec
        #traceback.print_stack(file=sys.stdout)
        print("whats wrong = ", self.basename, " ", self.task_local_step_pre)
    ...
    def set_task_ship_files_iter(self, fileconfigs, dest_dir, **kwargs):
        ...
        self.task_ship_files_prefix = {'basename': self.basename,
                                      'name': f"ship_files:_root_enter_",
                                      'task_dep': [],
                                      'actions': None
        ...
        # it is unsuable
        task_label_ship_files_prefix = f"{self.basename}:ship_files:_root_enter_"  #get_ref_name(self.task_ship_files_prefix)
        ...
        trec = {'basename': self.basename,
                'name': f"ship_files:_leaf_final_",
        ...
        self.final_task = f"{self.basename}:ship_files:_leaf_final_"
    ...
    def set_task_fetch_files_iter(self,
        ...
        self.task_fetch_files_prefix = {'basename': self.basename,
                                      'name': f"fetch_files:_root_enter_",
                                      'task_dep': [],
                                      'actions': None
        ...
        trec = {'basename': self.basename,
                    'name': f"fetch_files:_leaf_final_",
        ...
        self.final_task = f"{self.basename}:fetch_files:_leaf_final_"
        pass
    ...
    def set_super_task_seq(self, allroot_basename,
                           id_args= [], **kwargs):
        """
        task_dep  will be anchored on to prefix task
        """
        self.basename_super = f"""{allroot_basename}:{self.ipv6}"""
        if id_args:
            self.basename_super = f"""{self.basename}:{ "_".join([str(_) for _ in id_args])}"""
        ...
        self.rtas_taskseq_labels = [f"{self.basename}:_leaf_final_"]


        # all first task in the rtas
        # should have task dep to the prefix (aka setup) task
        print(f"task dep for super {allroot_basename} : ", kwargs.get('task_dep'))
        self.task_dep = [f"{allroot_basename}:_root_enter_", *kwargs.get('task_dep', [])]
    ...
    def set_new_subtask_seq(self, id_args= [], **kwargs):
        """
        ...
        # the super task
        self.task_dep = [f"{self.basename_super}:_leaf_final_", *kwargs.get('task_dep', [])]
        pass
